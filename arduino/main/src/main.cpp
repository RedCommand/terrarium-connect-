...
void database()
{
    ...
    if (client.connect(hostname, 4000))
    ...
    while (client.connected())
    {
        if (client.available())
        {
            ...
            if (!header)
            { // when header ended, put data into message variable
                message += c;
            }
        }
    }
...
{ // does not work because it is not a analog sensor
    byte data[9], addr[8];
...
void set_temp_zone_froide()
{
    ...
    get_temp_zone_froide(&current_temp_zone_froide);
...
void loop()
{
    ...
    for (int i = 0; i < 60; i++)
    {
        ...
        delay(1000); // wait 10 second
    }

    if (is_connected == false)
    { // if offline mode is enabled, trying to reconnect
        ethernet_init();
    }
}